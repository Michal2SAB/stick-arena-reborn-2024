class Physics {
  ...
  static checkForObstacles(playerPos, targetPlayerPos) {
    ...
    // Calculate number of steps between players
    const numSteps = Math.ceil(totalDistance / 50); // Rounding up to ensure we check all tiles
  
    // Calculate x and y increments for each step
    const xIncrement = xDistance / numSteps;
    const yIncrement = yDistance / numSteps;
  
    // Initialize variables for loop
    let currentX = playerPos.x;
    let currentY = playerPos.y;
  
    // Iterate through steps between players
    for (let i = 0; i < numSteps; i++) {
      // Convert current position to tile index
      const tileX = Math.floor(currentX / 50);
      const tileY = Math.floor(currentY / 50);
      const tileIndex = tileX + tileY * gameSettings.mapWidth;

      const tilemap = Constants.TILEMAP[tileIndex];
  
      // Check if tile is an obstacle
      if (tilemap.charAt(tilemap.length - 1 === "4")) {
